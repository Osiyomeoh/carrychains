...
const FindRoutesPage: React.FC = () => {
  ...
  // Apply filters when routes change - simplified to show all routes by default
  useEffect(() => {
    applyFilters();
  ...
  const applyFilters = () => {
    // Start with all routes - this ensures we see everything in state
    let filtered = [...state.routes];
    
    // Apply only location filters initially to ensure routes are visible
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {filteredRoutes.length === 0 && state.routes.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
...
const RouteCard: React.FC<RouteCardProps> = ({ 
  ...
  return (
    <motion.div 
      ...
      className="border border-gray-700 hover:border-indigo-500 bg-gray-800/50 rounded-xl overflow-hidden transition-colors duration-200"
    >
      <div className="p-5">
        <div className="flex flex-wrap md:flex-nowrap gap-6">
          ...
          <div className="w-full md:w-1/3 md:border-l border-gray-700 md:pl-6 flex flex-col justify-between">
            ...
            <Link
              to={`/route/${route.id}`}
              className="bg-indigo-500 hover:bg-indigo-600 text-white text-center font-medium py-3 px-4 rounded-lg flex items-center justify-center transition-colors"
            >
